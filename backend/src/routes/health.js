...
router.get("/health/db", async (_req, res) => {
  ...
  try {
    ...
    if (
      out.env.service_role_length < 100 ||
      out.env.service_role_prefix !== "eyJ"
    ) {
      out.hint =
        "Using a short/legacy key. Set the long Service Role Secret (JWT) in backend env and restart.";
    } else if (String(out.error).includes("Invalid API key")) {
      out.hint =
        "Key invalid for this project. Copy the Service Role Secret from the SAME Supabase project.";
...
router.get("/health/full", async (_req, res) => {
  ...
  res.setHeader("Access-Control-Allow-Headers", "Content-Type, Authorization"); // Allow specific headers
    const rest = await axios.get(
      `${env.SUPABASE_URL}/rest/v1/batches?select=id&limit=1`,
      {
        headers: {
          apikey: env.SUPABASE_SERVICE_ROLE_KEY,
          Authorization: `Bearer ${env.SUPABASE_SERVICE_ROLE_KEY}`,
