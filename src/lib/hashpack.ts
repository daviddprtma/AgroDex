...
export const wallet = {
  ...
  async connect() {
    ...
    if (!hc)
      return emit({ status: "error", message: "HashConnect not initialized" });
...
export const hashPackService = {
  async connectWallet(): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        reject(
          new Error(
            "Wallet connection timeout - Please approve the connection in HashPack",
          ),
        );
      }, 120000);
  ...
  getState() {
    ...
    return {
      ...
      accountId:
        state.status === "paired"
          ? state.sessionData?.accountIds?.[0] || null
          : null,
      network: "testnet",
    };
  },
